...
@section('content')
    ...
    <div class="col-md-12">
        <x-card type="info">
            ...
            <form style="padding:10px;" action="{{ route('dashboard.admin.customer.update',['id'=>$post->id]) }}" id="form-customer" method="post" role="form" class="form-horizontal " enctype="multipart/form-data">
                @method('PUT')
                <div class="form-group">
                     <input type="hidden" value="{{$post->id}}" name="id">
                     <div class="row">
                         <div class="col-md-1 col-sm-12">
                            <input type="text" style="padding:10px; margin: 10px 0px 16px 0px; height: 40px; border-radius: 7px; font-size: 16px;"class="form-control"   name="code" value="{{$post->code}}"  placeholder="کد مشتری">
                         </div>

                         <div class="col-md-3 col-sm-12">
                            <input type="text" style="padding:10px; margin: 10px 0px 16px 0px; height: 40px; border-radius: 7px; font-size: 16px;"class="form-control"   name="name" value="{{$post->name}}"  placeholder="نام و نام خانوادگی مشتری">
                         </div>

                         <div class="col-md-2 col-sm-12">
                             <input type="text" style="padding:10px; margin: 10px 0px 16px 0px; height: 40px; border-radius: 7px; font-size: 16px;"class="form-control"   name="tells" value="{{$post->tells}}"  placeholder="تلفن مشتری">
                         </div>

                        <div class="col-md-2 col-sm-12">
                            <input type="text" style="padding:10px; margin: 10px 0px 16px 0px; height: 40px; border-radius: 7px; font-size: 16px;"class="form-control"   name="tell" value="{{$post->tell}}"  placeholder="موبایل مشتری">
                         </div>
                         <div class="col-md-2 col-sm-12">
                            <input type="text" style="padding:10px; margin: 10px 0px 16px 0px; height: 40px; border-radius: 7px; font-size: 16px;"class="form-control"   name="job" value="{{$post->job}}"  placeholder="موضوع کسب و کار">
                         </div>
                         <div class="col-md-2 col-sm-12">
                            <input type="text" style="padding:10px; margin: 10px 0px 16px 0px; height: 40px; border-radius: 7px; font-size: 16px;"class="form-control"   name="referal" value="{{$post->referal}}"  placeholder=" معرف">
                         </div>
...
<tr>
  <form style="padding:10px;" action="{{ route('dashboard.admin.service.updateservice',['id'=>$item->id]) }}" method="post" id="form-id{{ $item->id }}" role="form" class="form-horizontal " enctype="multipart/form-data">
      @csrf
